/*
	This program was originally written for and copied from: Chapter 7 Drill and Exercises

    The final version of this copy is for Chapter 8, exercise 1

    CHANGE LOG:
        >changed Token Stream from global variable to local
        >added an istream& parameter to Token_stream constructor (7.8.2)
*/
...
class Token_stream 
{
	...
	public:
        ...
        Token_stream(istream& input) :full(0), buffer() { }  // make a Token_stream that reads from file
...
double sq_rt_handler(Token_stream& ts) 
{
	Token t = ts.get();
...
double pow_handler(Token_stream& ts) 
{
	Token t = ts.get();
	if (t.kind != '(') error("'(' expected");
	double d = expression(ts);
...
// deal with numbers, unary operators, parentheses, brackets, sqrts, and powers
double primary(Token_stream& ts)
{
	...
	case '-':
		return -primary(ts);          // returns unary minus
    case '+':           
        return + primary(ts);         // returns unary plus
	case number:
		return t.value;             // return the number's value
	case name:
		return Sym_Tbl.get(t.name); // return the variable's value of name t.name
	case sq_rt:										
		return sq_rt_handler(ts);	// handle square roots
	case power:		
		return pow_handler(ts);		// handle powers
	default:
...
double statement(Token_stream& ts)
{
	...
	switch (t.kind) {
	case let:
		return declaration(ts);   // begin steps to initialize variable with the name folowing 'let '
	default:
		ts.unget(t);
		return expression(ts);    // otherwise calculate math
	}
}
...
int main() 
{
    try {
        ...
        welcome_stmt();
        calculate(ts);    // loops on provided input until user quits or error
        return 0;
    } catch (exception& e) {
